...
12. **Partial `tool_result` submission (batched)** — v1 treats any mismatch between pending tool_use ids and incoming tool_result ids as a fresh blocking initial via JSONL rebuild, not as an in-place partial resolution. If clients need to stream results back in batches, this must become a first-class mode.
