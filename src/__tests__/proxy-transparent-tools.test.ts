...
describe("Phase 2: Message format preservation", () => {
  ...
  it("balanced-slices tool_result into JSONL; SDK prompt is a Continue. sentinel", async () => {
    ...
    // assistant turn) and the SDK prompt becomes the tool_result EmotionPrompt
    // user-nudge. This prevents the SDK's z77() deferred-tool detection from
    // forking the conversation and keeps the tool_result's bytes stable
