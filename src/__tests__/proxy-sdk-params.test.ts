...
describe("SDK param passthrough — header overrides", () => {
  ...
  it("all betas are stripped for claude-max profiles", async () => {
    const app = createTestApp()
    await post(app, BASE_BODY, { "anthropic-beta": "context-1m-2025-08-07" })
    expect(capturedOptions.betas).toBeUndefined()
  })

  it("comma-separated anthropic-beta header: all stripped for claude-max", async () => {
    const app = createTestApp()
  ...
  it("mixed betas: all stripped for claude-max", async () => {
    const app = createTestApp()
    await post(app, BASE_BODY, {
